...
function Menu() {
  ...
  const saveClientAndFullMenu = async () => {
    try {
      ...
      // 2️⃣ Save each Menu Context (Looping with index i)
      for (let i = 0; i < menuContexts.length; i++) {
        const ctx = menuContexts[i];

        // FIX: Extract the specific price/total for THIS row from the invoice state
        const rowData = invoiceData?.rows?.[i] || { price: 0, total: 0 };
        ...
        const contextResponse = await fetch('http://localhost:4000/api/menucontext', {
          ...
          body: JSON.stringify({
            ...
            members: ctx.members,
            buffet: ctx.buffet,
            // Now rowData is defined and pulled from the synchronized state
